...
if [ "$SKIP_PIP" = false ]; then
    ...
    readarray -t DETECTED_MISSING < <("${PYTHON_BIN}" - <<PYTEST
import sys
missing=[]
try:
    import tkinter
...
PYTEST
    ...
    if [ "${#DETECTED_MISSING[@]}" -gt 0 ]; then
        echo "[INFO] Foram detectadas possíveis dependências de sistema faltando: ${DETECTED_MISSING[*]}"
        if command -v apt >/dev/null 2>&1; then
            MANAGER="apt"
            PKGS_TO_INSTALL=()
            for m in "${DETECTED_MISSING[@]}"; do
                if [ "$m" = "tk" ]; then PKGS_TO_INSTALL+=(python3-tk); fi
                if [ "$m" = "pillow" ]; then PKGS_TO_INSTALL+=(libjpeg-dev libpng-dev); fi
                if [ "$m" = "opencv" ]; then PKGS_TO_INSTALL+=(libopencv-dev); fi
            done
        elif command -v pacman >/dev/null 2>&1; then
            MANAGER="pacman"
            PKGS_TO_INSTALL=()
            for m in "${DETECTED_MISSING[@]}"; do
                if [ "$m" = "tk" ]; then PKGS_TO_INSTALL+=(tk); fi
                if [ "$m" = "pillow" ]; then PKGS_TO_INSTALL+=(libjpeg-turbo libpng); fi
                if [ "$m" = "opencv" ]; then PKGS_TO_INSTALL+=(opencv); fi
            done
        elif command -v dnf >/dev/null 2>&1; then
            MANAGER="dnf"
            PKGS_TO_INSTALL=()
            for m in "${DETECTED_MISSING[@]}"; do
                if [ "$m" = "tk" ]; then PKGS_TO_INSTALL+=(python3-tkinter); fi
                if [ "$m" = "pillow" ]; then PKGS_TO_INSTALL+=(libjpeg-devel libpng-devel); fi
                if [ "$m" = "opencv" ]; then PKGS_TO_INSTALL+=(opencv); fi
            done
            ...
            PKGS_TO_INSTALL=()
            for m in "${DETECTED_MISSING[@]}"; do
                if [ "$m" = "tk" ]; then PKGS_TO_INSTALL+=(tcl-tk); fi
                if [ "$m" = "pillow" ]; then PKGS_TO_INSTALL+=(jpeg libpng); fi
                if [ "$m" = "opencv" ]; then PKGS_TO_INSTALL+=(opencv); fi
            done
        else
            MANAGER=""
        ...
        if [ "${#PKGS_TO_INSTALL[@]}" -gt 0 ]; then
            printf "[PERGUNTA] Deseja instalar os pacotes do sistema necessários agora? [y/N]: "
            read -r INSTALL_ANSWER || INSTALL_ANSWER="n"
            INSTALL_ANSWER="${INSTALL_ANSWER,,}"
            if [ "$INSTALL_ANSWER" = "y" ] || [ "$INSTALL_ANSWER" = "yes" ]; then
                if [ -n "${MANAGER}" ]; then
                    echo "[INFO] Executando instalador do sistema: ${MANAGER} ${PKGS_TO_INSTALL[*]}"
                    case "${MANAGER}" in
                        apt)
                            sudo apt update && sudo apt install -y "${PKGS_TO_INSTALL[@]}" || echo "[AVISO] Falha ao instalar alguns pacotes do sistema. Verifique sua conexão e permissões." ;;
                        pacman)
                            sudo pacman -Syu --noconfirm "${PKGS_TO_INSTALL[@]}" || echo "[AVISO] Falha ao instalar alguns pacotes do sistema." ;;
                        dnf)
                            sudo dnf install -y "${PKGS_TO_INSTALL[@]}" || echo "[AVISO] Falha ao instalar alguns pacotes do sistema." ;;
                        brew)
                            brew install "${PKGS_TO_INSTALL[@]}" || echo "[AVISO] Falha ao instalar alguns pacotes do sistema." ;;
                        *)
                            echo "[AVISO] Gerenciador desconhecido. Instale manualmente: ${PKGS_TO_INSTALL[*]}" ;;
                    esac
                else
                    echo "[AVISO] Nenhum gerenciador de pacotes suportado detectado. Instale manualmente: ${PKGS_TO_INSTALL[*]}"
                fi
            else
                echo "[INFO] Pulando instalação de pacotes do sistema. Você pode instalá-los manualmente posteriormente."
            fi
        fi
    fi
else
