...
"""Menu de contexto do Sidebar e renomeação inline."""
...
class InlineRename:
    """Gerencia a renomeação inline de arquivos/pastas no sidebar."""

    def __init__(self, sidebar, target_path: str, on_confirm_callback):
        self.sidebar = sidebar
        self.target_path = target_path
        self.on_confirm = on_confirm_callback

        self.target_button = sidebar.item_widgets.get(target_path)
    ...
    def _build_ui(self):
        ...
        if icon_text:
            self.icon_label = ctk.CTkLabel(
                self.inline_frame, text=icon_text, font=("Segoe UI", 12)
            )
            self.icon_label.pack(side="left", padx=(8, 4))

        self.entry_field = ctk.CTkEntry(
            self.inline_frame, height=22, font=("Segoe UI", 11),
            border_width=1, border_color="#3e4451", fg_color="#1d2026", text_color="#cccccc",
        )
        ...
        # Seleciona só o nome (sem extensão) para facilitar renomeação
        name_without_ext, ext = os.path.splitext(base_name)
        if os.path.isfile(self.target_path) and ext:
            self.entry_field.select_range(0, len(name_without_ext))
        else:
            self.entry_field.select_range(0, tk.END)

        self.entry_field.bind("<Return>", self._confirm)
        self.entry_field.bind("<Escape>", self._cancel)
        self.entry_field.bind("<FocusOut>", self._cancel)

        self.sidebar._bind_scroll_to_widget(self.inline_frame)
        if icon_text:
            self.sidebar._bind_scroll_to_widget(self.icon_label)
        self.sidebar._bind_scroll_to_widget(self.entry_field)
    ...
    def _confirm(self, event=None):
        ...
        if FileManager.rename_path(self.target_path, new_name):
            ...
            if hasattr(self.sidebar, "_show_error"):
                self.sidebar._show_error("Não foi possível renomear o item.")
...
class SidebarContextMenu:
    """Menu de contexto personalizado para o explorador de arquivos."""
    ...
    def _build_menu(self):
        ...
        self._add_item(
            "Renomear", self._menu_rename, icon="\uf044", **base_style,
            state="normal" if has_targets and not multi else "disabled",
        )

        self._add_separator()

        self._add_item(
            "Copiar Caminho Relativo", self._copy_relative_path,
            icon="\uf0c5", **base_style, state="normal" if has_targets else "disabled",
        )
        self._add_item(
            "Copiar Caminho Absoluto", self._copy_absolute_path,
            icon="\uf0c5", **base_style, state="normal" if has_targets else "disabled",
        )

        self._add_separator()

        delete_style = {**base_style, "text_color": "#e06c75", "hover_color": "#4a2530"}
        self._add_item(
            "Excluir", self._menu_delete, icon="\uf1f8", **delete_style,
            state="normal" if has_targets else "disabled",
        )
        ...
        self._add_item(
            "Add (Stage)", self._git_add, icon="\uf067", **base_style,
            state="normal" if has_targets else "disabled",
        )
        self._add_item(
            "Reset (Unstage)", self._git_reset, icon="\uf0e2", **base_style,
            state="normal" if has_targets else "disabled",
        )
        self._add_item(
            "Ver Diff", self._git_diff, icon="\uf126", **base_style,
            state="normal" if has_targets else "disabled",
        )
    ...
    def _add_item(self, label, command, icon="", **kwargs):
        is_disabled = kwargs.pop("state", "normal") == "disabled"
        btn = ctk.CTkButton(
            self.menu_frame, text=f"{icon}    {label}",
            command=lambda: self._action(command),
    ...
    def _add_separator(self):
        sep = ctk.CTkFrame(
            self.menu_frame, height=1,
            fg_color=self.theme.get("menu_separator", "#3e4451"),
        )
    ...
    def _copy_relative_path(self):
        ...
        paths = "\n".join(
            os.path.relpath(t, root) if root else t for t in self._menu_targets
        )
    ...
    def _menu_delete(self):
        ...
        dialog.transient(self.sidebar.winfo_toplevel())
        ...
        def do_delete():
            for target in self._menu_targets:
                if not FileManager.delete_path(target):
                    if hasattr(self.sidebar, "_show_error"):
                        self.sidebar._show_error(
                            f"Não foi possível excluir: {os.path.basename(target)}"
                        )
            self.sidebar.refresh_explorer()
            dialog.destroy()

        ctk.CTkButton(
            btn_frame, text="Excluir", fg_color="#e06c75",
            hover_color="#be5046", command=do_delete,
        ).pack(side="left", padx=10)
        ctk.CTkButton(
            btn_frame, text="Cancelar", fg_color="gray", command=dialog.destroy,
        ).pack(side="left", padx=10)
    ...
    def _on_rename_success(self, old_path: str, new_path: str):
        ctx = AppContext()

        # Atualiza project_root se necessário
        if ctx.project_root == old_path:
            ctx.project_root = new_path

        # Atualiza abas afetadas
        if ctx.tab_manager:
